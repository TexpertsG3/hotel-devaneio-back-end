...
public class ServicoAdicionalDAO {
	...
	public void insere(ServicoAdicional servicoAdicional) {
		...
		try (Connection conn = ConnectionFactory.criaConexao(); 
			...
			st.execute();
			System.out.println("ServiÃ§o " + servicoAdicional.getNomeServico() + " adicionado com sucesso.");
	...
	public void delete(ServicoAdicional servicoAdicional) {
        String sql = "delete from servico_adicional where id_servico_adicional = ?";

        try (Connection conn = ConnectionFactory.criaConexao();
             PreparedStatement st = conn.prepareStatement(sql)) {
            
            st.setInt(1, servicoAdicional.getIdServico());

            st.execute();
            System.out.println("Serviço " + servicoAdicional.getNomeServico() + " deletado com sucesso.");
        } catch (SQLException e) {
            System.err.println(ERRO_CONEXAO + "\n" + e.getMessage());
        }
    }
